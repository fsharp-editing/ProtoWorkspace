...
type FileSystem(editorBufferTracker : IBufferTracker) = 
    ...
    let getOpenDocContent (fileName : string) = 
        editorBufferTracker.TryFindEditorBuffer fileName |> Option.map (fun doc -> doc.Text |> doc.Encoding.GetBytes)
    interface IFileSystem with
        ...
        member __.ReadAllBytesShim fileName = 
            getOpenDocContent fileName |> Option.getOrTry (fun () -> defaultFileSystem.ReadAllBytesShim fileName)
        
        member __.GetLastWriteTimeShim fileName = 
            editorBufferTracker.TryFindEditorBuffer fileName
            |> Option.bind (fun doc -> 
                   if doc.IsDirty then Some doc.LastChangeTime
                   else None)
